...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (ClientPrefs.data.gameplaySettings.get('modchart') == true)
		{
		...
		if (ClientPrefs.data.hitsoundVolume > 0)
			precacheList.set('hitsound-${ClientPrefs.data.ht}', 'sound');
		...
		if (ClientPrefs.data.windows)
			winapi.WindowsLua.loadLua();
	...
	function animIconCheck(isOpponent:Bool = false)
	{
		if (isOpponent)
		{
			if (dad.hasAnimatedIcon)
			{
				...
				iconP2.scale.set(dad.iconScale[0], dad.iconScale[1]);
				iconP2.updateHitbox();
	...
	function updateIconOffset(isOpp:Bool = false, isLosing:Bool = false)
	{
		if (!isOpp)
		{
			if (isLosing)
			{
				...
				@:privateAccess {
					iconP1.iconOffsets = [boyfriend.iconOffsets.loseX, boyfriend.iconOffsets.loseY];
				}
			}
			else
			{
		...
		else
		{
			if (isLosing)
			{
				...
				@:privateAccess {
					iconP2.iconOffsets = [dad.iconOffsets.loseX, dad.iconOffsets.loseY];
				}
			}
			else
			{
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (ClientPrefs.data.hitsoundVolume > 0 && !note.hitsoundDisabled && ClientPrefs.data.ht != "Baldi")
			{
		...
		for (i in 0...achievesToCheck.length)
		{
			...
			if (!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled)
			{
				...
				switch (achievementName)
				{
					...
					case 'toastie':
						if (/*ClientPrefs.data.framerate <= 60 &&*/ !ClientPrefs.data.shaders && ClientPrefs.data.lowQuality
